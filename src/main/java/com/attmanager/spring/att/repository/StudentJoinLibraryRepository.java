...
public interface StudentJoinLibraryRepository extends JpaRepository<StudentJoinLibrary, Long> {
    ...
    @Query(value = "select year.year, count(students.year) as quantity from attmanger.student_join_library " + 
    ...
    @Query(value = "select departments.department, count(students.department) as quantity from attmanger.student_join_library " +
